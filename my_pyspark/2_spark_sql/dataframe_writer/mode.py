...
# +----+---+
# 以默认参数写入json文件
df.write.mode(saveMode="error").json("output/json")
# ls -l output/json/
# 总用量 4
...
# 先清空文件夹再写入，并压缩文件
df.write.mode(saveMode="overwrite").json("output/json", compression='gzip2')
...
df.write.mode(saveMode="overwrite").json("output/json", compression='gzip')
# ls -l output/json/
# 总用量 4
